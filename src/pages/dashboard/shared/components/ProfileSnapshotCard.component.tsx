...
const ProfileSnapshotCard = () => {
	...
	const isOpen = data.profile.isOpenToOpportunities === 'open';
	return (
		<CardLayout className="p-6">
			...
			<div className="space-y-4">
				...
				<div className="flex justify-between border-b border-slate-100 pb-2">
					...
					<span
						className={`text-xs px-2 py-0.5 rounded-full font-bold ${isOpen ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
						{isOpen ? 'OPEN TO WORK' : 'HIRED'}
					</span>
				</div>
				<div className="flex justify-between border-b border-slate-100 pb-2">
